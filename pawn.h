#ifndef PAWN_H
#define PAWN_H
#include "piece.h"

class DefaultBoard;

class Pawn : public Piece {
    private:
        DefaultBoard* b;
    public:
        Pawn(char name, int curX, int curY, std::string teamName, bool blockable, DefaultBoard* theBoard, std::string captureDir);

        // checks if a given move is valid for the piece
        // checking if enpassantable is valid here too
        bool isValidMove(int xMove, int yMove) override;

        /* returns all valid moves possible for the piece
            - each move is separated by a "|"
            - ex: 2,3|4,5|3,2|
        */
        std::vector<std::shared_ptr<Move>> allValidMoves() override;
        bool isPromotable() override;
        bool doneOneMove();
        bool enpessantable() override;
        bool isValidCaptureMove(int xMove, int yMove) override;
        bool inLineOfSight(int XMove, int yMove) override;

};
#endif
